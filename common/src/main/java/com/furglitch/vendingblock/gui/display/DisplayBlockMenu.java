...
public class DisplayBlockMenu extends AbstractContainerMenu {
    ...
    public DisplayBlockMenu(int id, Inventory inv) {
        this(id, inv, new SimpleContainer(1), new SimpleContainerData(1));
    }

    public DisplayBlockMenu(int id, Inventory inv, Container container, ContainerData data) {
        super(MenuRegistry.DISPLAY_MENU.get(), id);
    ...
    private static final int CONTAINER_INVENTORY_ROW_COUNT = 1;
    private static final int CONTAINER_INVENTORY_COLUMN_COUNT = 1;
    private void addContainerInventory(Container cont) {
        for (int i = 0; i < CONTAINER_INVENTORY_ROW_COUNT; i++) {
            for (int j = 0; j < CONTAINER_INVENTORY_COLUMN_COUNT; j++) {
                int slotIndex = j + i * 3;
                this.addSlot(new Slot(cont, slotIndex, 62 + (j * 18), 35 + (i * 18)) {
                    @Override
        ...
        if (cont instanceof VendorBlockEntity be) {
            this.addSlot(new GhostFilterSlot(be, 0, 98, 35));
        } else {
            this.addSlot(new GhostFilterSlot(null, 0, 98, 35));
        }
    }
